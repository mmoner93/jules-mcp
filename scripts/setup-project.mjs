...
try {
  ...
  const projectInput = (await rl.question("Enter absolute path to your project root: ")).trim();
  if (!projectInput) {
  ...
  const stateFile = path.join(projectRoot, ".jules-sessions.json");
  ...
  console.log(`Updated project MCP config: ${configPath}`);
